    ...
    def handle_line(self, json_data):
        ...
        match action:
            case 'HOST':
                ...
                addr = self.transport.getPeer()
                logging.info(
                        f'New game room created by {addr.host} with \
                        game_code: {gc}')
                self.factory.game_rooms[gc] = Room(game_code=gc,
                                                   players=[self.transport])
                res = {
            ...
            case 'CANCEL':
                ...
                addr = self.transport.getPeer()
                if addr not in [player.getPeer() for player
                                in self.factory.game_rooms[gc].players]:
                ...
                try:
                    ...
                    logging.error(
                        f'Unknown error occured while trying to cancel game: \
                        {e}')
